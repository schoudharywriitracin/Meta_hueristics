...
# General parameters
PopulationSize = 50  # Number of hawks
Iterations = 30  # Number of HHO iterations
FuncAgain = 1  # Number of times to repeat each function
Export = True  # Whether to export results to CSV
ExportToFile = "HHO_Optimization_" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".csv"
...
if assigned_station:
    ...
    print(f"Processing station: {stn_id}")

    # Clean features and target to ensure no NaN, infinity, or extreme values
    X = station_df[['WND', 'SLR', 'HMD', 'TMPmin', 'TMPmax']].values
    y = station_df['PCP'].values

    # Replace NaN and infinity in X and y with 0.0 and validate
